...
export async function* decodeMessages(source) {
...
export function encodeMessage(opcode, data, maskData) {
    const fin = true;
    const message = new Uint8ArrayList(Uint8Array.from([
        (fin ? 128 : 0) | OP_CODES[opcode]
    ]));
    const length = data?.byteLength ?? 0;
    if (length < 126) {
        message.append(Uint8Array.from([
            length | (maskData === true ? 128 : 0)
        ]));
    }
    else if (length < 65_535) {
        const l = new Uint8ArrayList(new Uint8Array(3));
        l.set(0, 126 | (maskData === true ? 128 : 0));
        l.setUint16(1, length);
        message.append(l);
    }
    else if (length < 18_446_744_073_709_552_000) {
        const l = new Uint8ArrayList(new Uint8Array(9));
