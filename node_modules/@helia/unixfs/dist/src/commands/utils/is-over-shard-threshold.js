...
import { hamtBucketBits } from "./hamt-constants.js";
/**
 ...
 * a regular directory.
 */
export async function isOverShardThreshold(node, blockstore, options) {
    if (node.Data == null) {
    ...
    let strategy = options.shardSplitStrategy ?? 'links-bytes';
...
function calculateNodeSize(unixfs, node) {
    let size = dataFieldSerializedSize(unixfs.mode, unixfs.mtime);
    for (const link of node.Links) {
        size += linkSerializedSize(utf8ByteLength(link.Name ?? ''), link.Hash.byteLength, Number(link.Tsize ?? 0));
    }
    return size;
}
...
async function calculateShardSize(node, current, max, blockstore, options) {
    if (current > max) {
        return max;
    }
    if (node.Data == null) {
